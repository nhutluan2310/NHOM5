...
<div class="container">
    <div class="header">
        <a href="index.php"><img src="Images/bannerDL.png" width="100%" height="80%" alt="BannerDL"></a>
        <div class="clear"></div>
    </div>
    <div class="body" >
        <div class="body-left" style="background: #FDFDCA;">
            <div class="content" >
                <h3 style="background:#ff4c4c; padding:7px;">Danh mục sữa bán chạy</h3>
                <?php
                    $query="SELECT `sua`.`Ma_sua`, `sua`.`Ten_sua` FROM sua CROSS JOIN hang_sua CROSS JOIN loai_sua CROSS JOIN ct_hoadon WHERE 
                    `sua`.`Ma_hang_sua`=`hang_sua`.`Ma_hang_sua` and `sua`.`Ma_loai_sua`= `loai_sua`.`Ma_loai_sua` 
                    and `sua`.`Ma_sua` = `ct_hoadon`.`Ma_sua` GROUP BY sua.Ma_sua ORDER BY `ct_hoadon`.`So_luong` 
                    DESC LIMIT 5;";
                    ...
                    if($numRows<>0)
                    {
                        ...
                        while($rows=mysqli_fetch_array($result)){
                            ...
                            echo "<a href=" . $_SERVER['PHP_SELF'] . "?milk=" . "{$rows['Ma_sua']}" . ">&nbsp;{$rows['Ten_sua']}</a>";
                            echo "</li>";
                            $num++;
                        }
                        echo "</ul>";
            ...
            <?php
                ...
                if(isset($_GET['milk'])){
                    ...
                    $query1="SELECT `sua`.`Hinh` as'Hình',`sua`.`Ten_sua` as 'Tên Sữa', `sua`.`TP_Dinh_Duong` as 
                        ...
                        `sua`.`Don_gia` as'Đơn giá', `hang_sua`.`Ten_hang_sua`, SUM(`ct_hoadon`.`so_luong`) as 
                        'Tổng số lượng' FROM sua CROSS JOIN hang_sua CROSS JOIN loai_sua CROSS JOIN ct_hoadon WHERE 
                        `sua`.`Ma_hang_sua`=`hang_sua`.`Ma_hang_sua` and `sua`.`Ma_loai_sua`= `loai_sua`.`Ma_loai_sua` 
                        and `sua`.`Ma_sua` = `ct_hoadon`.`Ma_sua` and (`sua`.`Ma_sua` = '$milk') GROUP BY sua.Ma_sua ORDER BY `ct_hoadon`.`So_luong`";
                    ...
                    if($numRows<>0)
                    {
                        ...
                        { 
                            echo "<tr style='background:#FEFD98;'>
                                <th id='title' colspan='3' align='left'><h2>{$rows['Tên Sữa']}</h2></th>
                                </tr>";
                            echo "<tr style='background:white;'>
                                <td colspan='2' align='center'><img src=Hinh_sua/{$rows['Hình']} height="."400px"." width="."400px"."></td>
                                <td class='txt'>";
