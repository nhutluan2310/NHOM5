        ...
        <div class="body-center">
                    ...
                    <table class="Tblsearch">
                        <tr>
                            <td align="center">
                                ...
                                <label for="loaisua">Hãng sữa:</label>
                        ...
                        <tr>
                            <td align="center">
                                ...
                                <input type="text" name="search" value="<?php if(isset($_GET['search'])) echo $tim;?>" required title="Vui lòng nhập tên sữa cần tìm!" id="search">
                        ...
                        <tr>
                            <td align="center" style="padding:10px;">
                                <?php
                                    ...
                                    if(isset($_GET['submit'])){
                                        $query1="SELECT `sua`.`Hinh` as'Hình',`sua`.`Ten_sua` as 'Tên Sữa', `sua`.`TP_Dinh_Duong` as 'Thành phần dinh dưỡng', `sua`.`Loi_ich` as'Lợi ích',`sua`.`Trong_luong` as 'Trọng lượng', `sua`.`Don_gia` as'Đơn giá', `hang_sua`.`Ten_hang_sua`
                                        FROM sua CROSS JOIN hang_sua CROSS JOIN loai_sua WHERE `sua`.`Ma_hang_sua`=`hang_sua`.`Ma_hang_sua` and `sua`.`Ma_loai_sua`= `loai_sua`.`Ma_loai_sua` and (`sua`.`Ma_loai_sua` like '%$loaisua%' and `sua`.`Ma_hang_sua` like '%$hangsua%'
                                        and ( `sua`.`Ten_sua` like '%$tim%' ))  LIMIT $offset, $rowsPerPage;";
                                        $result1 = mysqli_query($conn,$query1);
                                        if (!$result1 ) die ('<br> <b>Query failed</b>');
                                        $numRows= mysqli_num_rows($result1);
                                        if($numRows<>0){
                                            echo "<b>Có {$numRows} sản phẩm được tìm thấy!</b>";
                                        }else{
                                            echo "<b>Không tìm thấy sản phẩm<b>";
                                        }
                                    }
                                    mysqli_close($conn);
                                ?>
            ...
            <?php
                ...
                if(isset($_GET['submit'])){
                    ...
                    if($numRows<>0)
                    {
                        ...
                        { 
                            echo "<tr style='background:#FEFD98;'>
                                <th id='title' colspan='3' align='left'>{$rows['Tên Sữa']} - {$rows['Ten_hang_sua']}</th>
                                </tr>";
                            echo "<tr style='background:white;'>
                                <td colspan='2' align='center'><img src=Hinh_sua/{$rows['Hình']} width="."200"."></td>
                        ...
                        if ($_GET['page'] > 1){
                            echo "<a  href=" .$_SERVER['PHP_SELF']."?page=".($_GET['page']-1)."&loaisua=".($_GET['loaisua'])."&hangsua=".($_GET['hangsua'])."&search=".($_GET['search']).">Back</a> "; //gắn thêm nút Back
                        }
                        for ($i=1 ; $i<=$maxPage ; $i++)
                        {
                            if ($i == $_GET['page'])
                            {
                                ...
                                echo "<a  href=" . $_SERVER['PHP_SELF'] . "?page=" . $i ."&loaisua=".($_GET['loaisua'])."&hangsua=".($_GET['hangsua'])."&search=".($_GET['search']). ">Trang " . $i . "</a> ";
                            }
                        }
                        if ($_GET['page'] < $maxPage) {
                            echo "<a href=" . $_SERVER['PHP_SELF'] . "?page=" . ($_GET['page'] + 1)."&loaisua=".($_GET['loaisua'])."&hangsua=".($_GET['hangsua'])."&search=".($_GET['search']) . ">Next</a>";  //gắn thêm nút Next
                        }
                        echo"</div>";
                    }
                }
                    ...
                    //phan trang
                    $query="SELECT `sua`.`Hinh` as'Hình',`sua`.`Ten_sua` as 'Tên Sữa', `sua`.`TP_Dinh_Duong` as 'Thành phần dinh dưỡng', `sua`.`Loi_ich` as'Lợi ích',`sua`.`Trong_luong` as 'Trọng lượng', `sua`.`Don_gia` as'Đơn giá', `hang_sua`.`Ten_hang_sua`
                    ...
                    if($numRows<>0)
                    {
                        ...
                        while($rows=mysqli_fetch_array($result))
                        {
                            ...
                            foreach ($field_info as $val) {                                  
                                if($val->name=="Thành phần dinh dưỡng"){
                                    echo "<b>".$val->name .":</b><br>";
                                    echo "<p>{$rows['Thành phần dinh dưỡng']}</p>";
                                }
                                if($val->name=="Lợi ích"){
                                    echo "<br><b>".$val->name .":</b><br>";
                                    echo "<p>{$rows['Lợi ích']}</p>";
                                }  
                                if($val->name=="Trọng lượng"){
                                    echo "<br><b>". $val->name .":</b> ";
